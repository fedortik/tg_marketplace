...
@router.callback_query(F.data.startswith('main_put_'))
async def callbacks_num(callback: types.CallbackQuery, state: FSMContext, bot: Bot):
    ...
    await state.set_state(basket.enter_count)


@router.message(StateFilter(basket.enter_count))
async def cmd_start(message: types.Message, state: FSMContext, bot: Bot):
    ...
    db.add_basket(message.chat.id, data['goods_id'], message.text)
